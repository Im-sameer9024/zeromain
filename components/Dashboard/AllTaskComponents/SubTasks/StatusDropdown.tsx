...
const StatusDropdown: React.FC<StatusDropdownProps> = ({
  ...
  const statusMutation = useMutation({
    mutationFn: (payload: UpdateStatusPayload) =>
      updateSubTaskStatus(subtaskId, payload),
  ...
  useEffect(() => {
    if (isRunning && isAssignedUser) {
      // Only run timer if user is assigned
  ...
  return (
    <>
              ...
              {!isRunning ? (
                <button
                  ...
                  className={`border-red-500 text-red-500 border bg-transparent hover:bg-red-50 text-xs py-1 px-2 rounded-sm ${
                    !isAssignedUser ? "opacity-50 cursor-not-allowed" : ""
                  }`}
                >
                  Start
                </button>
      ...
      <Popup
        openModal={showConfirmModal && isAssignedUser}
