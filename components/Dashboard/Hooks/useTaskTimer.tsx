...
const useTaskTimer = (taskId: string, initialStatus: string, initialTime: number = 0) => {
  ...
  const updateStatus = useCallback(async (newStatus: string) => {
    try {
      ...
      const response = await axios.put(
        `https://task-management-backend-kohl-omega.vercel.app/api/tasks/update-status/${taskId}`,
        {
